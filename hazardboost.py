# -*- coding: utf-8 -*-
"""
Created on Sun Jun 25 14:21:56 2017
...
def hazardBoost(delta, lotraj, maxsplits=2, numtrees=100, numtimepartitions=20, numvarpartitions=20, shrink=0.1, GlobalPartition = True, verbose=0, cat=None):
...
def plot(estimator, varIndices, var1range, var2range, ntreelimit = np.Inf, plotPoints=500, rstride = 10, cstride = 10, clip=False, clipValue = None):
...
def plotData(estimator, newdata, var1v, var2v,ntreelimit = np.Inf, plotPoints=500, rstride = 10, cstride = 10, clip=False, clipValue = None):
    import matplotlib.pyplot as plt
    from mpl_toolkits.mplot3d import Axes3D
    from matplotlib import cm
    from matplotlib.ticker import LinearLocator, FormatStrFormatter
...
def variableImportance(estimator, colnames=None):
    ...
    varImp = {k: v/max(varImp.values()) for k,v in varImp.iteritems()}
